...
print("PERCEPTION: s_hat, q: ", s_hat_perception, q_perception)

# ------------------------- Hardcoding grasp poses --------------------------
# Left hand: Set initial pose to a default value
left_eef_pos_pre = np.array([ 0.56033371,  0.20095334,  0.79674643])
left_eef_quat_pre = np.array([ 0.43567574, -0.54655992,  0.44715073, -0.5581354])
left_eef_pos = np.array([ 0.54033371,  -0.01095334,  0.79674643])
...
for epoch in range(3):
    ...
    if epoch == 0:
        ...
        q_new = elite_traj['q']
        print("Elite traj: s_hat, q: ", s_hat_new, q_new)
    if epoch > 0:
        ELITE_TRAJ_THRESHOLD = 3
        noise_s_hat_lis, noise_q_lis = [], []
        full_list_sorted_by_ft = get_elite_trajs(trajs)
        ...
        for traj in full_list_sorted_by_ft[:upper_bound]:
            ...
            print("Elite traj: s_hat, q: ", trajs[traj_number_temp]['s_hat'], trajs[traj_number_temp]['q'])
        ...
        if len(noise_s_hat_lis) != 0:
            ...
            mu_x_q, sigma_x_q, mu_y_q, sigma_y_q, mu_z_q, sigma_z_q = updateGaussianParams(noise_s_hat_lis)
